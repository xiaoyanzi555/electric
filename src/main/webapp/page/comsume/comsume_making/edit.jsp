...
<script type="text/javascript">
    $(function () {
        ...
        //设置工作单据的下拉选中
        $("#work_receipt").combobox("select","{{work_documents}}");
        //设置显示
        $.ajax({
            type:'get',
            url:'/users/{{distribution_id}}',
            success:function(result){
                if(result.code==200){
                    $("#distribution_name").html(result.data.uname)
                }
            }
        })
        //设置消缺员的显示
        ...
        $.ajax({
            ...
            success:function (result) {
                if(result.code==200){
                    ...
                    //转化为字符串进行附加
                    $("#consomers").html(nameArray.join(","))
                    //设置任务负责人
                    $("#task_user").val(nameArray[0]);
                }
            }
        })
        //通过中间表查询缺陷的编号
        $.ajax({
            type:'get',
            url:'/defects',
            headers:{"comsume":"defects"},
            data:{id:"{{id}}",is_deal:1},
            success:function (result) {
                if(result.data){
                    //得到所有的缺陷然后附加
                    var defects=result.data;
                    $.each(defects,function (i) {
                        $("#showDefect").datagrid("appendRow",defects[i])
                    })
                }
            }
        })
...
<div id="ConsumeData">
    <div class="easyui-dialog" title="选择消缺员" id="small_dialog" style="width:450px"
        ...
        <form id="ComsumeDataForm" action="/consumeTask" method="put">
            <input type="hidden" name="id" value="{{id}}">
            <table>
                <tr>
                    <td>
                        <strong>任 务 编 码</strong>
                        <input style="width: 200px;height: 28px" name="consume_task_code" value="{{consume_task_code}}">
                    </td>
                    <td>
                        <strong style="margin-left: 30px">任务名称</strong>
                        <input style="width: 200px;height: 28px" name="task_name" value="{{task_name}}">
                    </td>
                    <td>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <label style="display:inline-block;width: 200px" id="distribution_name"></label>
                        <input type="hidden" name="distribution_id" value="{{distribution_id}}">
                    </td>
                    <td>
                        <strong style="margin-left: 30px">下发日期</strong>
                        <input id="distribute_time" style="width: 200px;height: 28px" name="distribute_time" value="{{distribute_time}}">
                    </td>
                </tr>
                <tr>
                    <td>
                        <strong>任务描述</strong>
                        <textarea cols="18" rows="8" name="task_description">{{task_description}}</textarea>
                    </td>
                    <td>
                        <strong>备注</strong>
                        <textarea cols="18" rows="8" name="remark">{{remark}}</textarea>
                    </td>
                    <td>
                        <strong>消缺员</strong>
